...
class RoomHandler:
    ...
    def add_user_to_room(self, user: Session, room_name: str, password=""):
    ...
    def remove_user_from_room(self, user: Session, room: Room):
        if room in self.__rooms and room.has_user(user):
            room.remove_user(user)

    def is_user_in_room(self, user: Session) -> bool:
        for room in self.__rooms:
            if room.has_user(user):
                return True
        return False

    def get_room_from_user(self, user: Session) -> Room or None:
